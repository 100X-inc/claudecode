...
**著者:** WorldDownTown (GENDA)
**公開日:** 2025年12月4日
**URL:** https://zenn.dev/genda_jp/articles/70aa9a74ac1e62
**タグ:** GitHub, Claude, Tech

---

## 目次

1. [はじめに - デフォルト設定の課題](#はじめに---デフォルト設定の課題)
2. [課題](#課題)
3. [本家の利用方法](#本家の利用方法)
4. [改善のポイント](#改善のポイント)
5. [実際に導入したもの（抜粋）](#実際に導入したもの抜粋)
6. [導入後の効果](#導入後の効果)
7. [まとめ](#まとめ)
...
[anthropics/claude-code-action](https://github.com/anthropics/claude-code-action) は GitHub Actions で手軽に AI コードレビューを導入できます。
...
**参照:** [anthropics/claude-code-action/.github/workflows/claude-review.yml](https://github.com/anthropics/claude-code-action/blob/v1.0.21/.github/workflows/claude-review.yml#L20-L27)
...
PR 作成
    ↓
ワークフロー claude-review.yml 開始
    ↓
スラッシュコマンド /review-pr 実行
    ↓
サブエージェントが複数の視点をレビュー
    ├── コード品質
    ├── パフォーマンス
...
**参照:** [anthropics/claude-code-action/.claude/commands/review-pr.md](https://github.com/anthropics/claude-code-action/blob/v1.0.21/.claude/commands/review-pr.md)
...
![インラインコメントとSuggestionの例](suggestion_with_inline_comment.png)
...
#### code-quality-reviewer.md (コード品質レビュアー)
...
#### security-code-reviewer.md (セキュリティコードレビュアー)
...
#### documentation-accuracy-reviewer.md (ドキュメント正確性レビュアー)
...
1. **複数の視点でのレビュー**: 人間が見落としがちなエッジケースや、可読性の観点を AI が補完してくれます。

2. **指摘の見落とし減少**: インラインコメントになったことで、指摘箇所が明確になり、見逃しが激減しました。

3. **修正コスト削減**: 具体的な修正案 (Suggestion) が提示されることもあり、ボタン一つでコミットできる場合もあります。

4. **レビュー時間の短縮**: 人間のレビュアーは、AI が指摘済みの単純なミスをスルーして、設計やロジック、仕様の整合性といった、より高度なレビューに集中できるようになりました。
...
- **募集要項・カジュアル面談:** https://genda.jp/careers/
- **株式会社GENDA コーポレートサイト:** https://genda.jp/
- **GENDA Creators Blog:** https://blog.genda.jp/creators/

---

## 添付ファイル

このドキュメントと同じディレクトリに以下のファイルが保存されています：

- `full_page.png` - 記事全体のスクリーンショット
- `section_*.png` - 各セクションのスクリーンショット
- `image_*.png` - 記事内の画像（ダウンロード済み）
- `article_data.json` - 構造化データ
- `full_text.txt` - プレーンテキスト
- `article.html` - HTML形式
